...
*/
/* 定义 LCD 的命令和数据位. */
//#define UCGUI_LCD_CMD LCD_BASE /* 地址为 LCD 的 CMD */
//#define UCGUI_LCD_DATA (((LCD_BASE >> 1) + 1) << 1) /* 地址为 LCD 的 RAM */
...
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, LCD_PIXELINDEX PixelIndex) {
    ...
    GUI_USE_PARA(PixelIndex);
    {
      ...
      // TBD by customer...
      //
			lcd_draw_point(x,y,PixelIndex); /* 打点函数 */
    }
...
static LCD_PIXELINDEX _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
  LCD_PIXELINDEX PixelIndex;
    ...
    GUI_USE_PARA(y);
    {
      ...
      // TBD by customer...
      //
      PixelIndex = lcd_read_point(x,y); /* 读函数 */
    }
    #if (LCD_MIRROR_X == 0) && (LCD_MIRROR_Y == 0) && (LCD_SWAP_XY == 0)
      #undef xPhys
      #undef yPhys
...
static void _FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1) {
  ...
  if (GUI_pContext->DrawMode & LCD_DRAWMODE_XOR) {
    ...
    lcd_fill(x0,y0,x1,y1,LCD_COLORINDEX); /* 填充区域 */
  }
}
...
static void _DrawBitLine16BPP(GUI_DEVICE * pDevice, int x, int y, U16 const * p, int xsize) {
	...
	for (;xsize > 0; xsize--, x++, p++)
	{
	if(xsize>lcddev.width)y++;
	lcd_set_window(x,y,lcddev.width-xsize,y);
	pixel = *p;
