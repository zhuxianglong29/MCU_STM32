...
void text_show_string_middle(uint16_t x, uint16_t y, char *str, uint8_t size, uint16_t width, uint16_t color)
{
    ...
    {
        text_show_string(x, y, tftlcd_data.width, tftlcd_data.height, str, size, 1, color);
    }
    else
    {
        strlenth = (width - strlenth) / 2;
        text_show_string(strlenth + x, y, tftlcd_data.width, tftlcd_data.height, str, size, 1, color);
    }
}
