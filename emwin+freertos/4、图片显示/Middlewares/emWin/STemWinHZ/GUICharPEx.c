...
static void GUI_GetDataFromMemory(const GUI_FONT_PROP GUI_UNI_PTR *pProp, U16P c)
{
    ...
    csize = (size/8+((size%8)?1:0))*(size);   /* 得到字体一个字符对应点阵集所占的字节数 */
      ...
      {
          case 12:
              norflash_read(GUI_FontDataBuf, foffset + ftinfo.f12addr, csize);
              break;

          case 16:
              norflash_read(GUI_FontDataBuf, foffset + ftinfo.f16addr, csize);
              break;

          case 24:
              norflash_read(GUI_FontDataBuf, foffset + ftinfo.f24addr, csize);
              break;
      } 
    }
}
